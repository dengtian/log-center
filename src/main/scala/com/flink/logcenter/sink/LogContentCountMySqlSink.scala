...
class LogContentCountMySqlSink extends RichSinkFunction[CountLog] {
  ...
  override def invoke(value: CountLog, context: SinkFunction.Context[_]): Unit = {
    ...
    if (hour >= 0 && this.logType.nonEmpty) {
      ...
      val i = Db.use().count(Entity.create("content_hour")
      ...
      if (i < 1) {
        Db.use().insert(Entity.create("content_hour")
          .set("content_id", value.contentId)
          .set("create_by_id", value.createById)
          .set("behavior_type", logType.toLowerCase())
